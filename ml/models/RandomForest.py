...
#     def __init__(self, model_name, n_estimators=100, max_depth=None, min_samples_split=2, min_samples_leaf=1):
#         """
...
#         """
#         Prepares the dataset for Random Forest training.
#         """
#         X, y = prepare_data(df)
...
class RandomForest(BaseModel):
    ...
    def calculate_metrics(self, y_actual, y_pred, is_log=True):
        ...
        mse = mean_squared_error(y_actual, y_pred)
        rmse = np.sqrt(mse)
        # Sử dụng mean_absolute_percentage_error từ sklearn
        mape = np.mean(np.abs((y_actual - y_pred) / (y_actual + epsilon))) * 100
        return mse, rmse, mape
    ...
    def train(self, X_train, y_train, X_test, y_test, is_log=False, tuning_method="random_search"):
        ...
        # Cross-Validation với 20 folds
        logging.info("Performing 20-Fold Cross-Validation...")
        cv_scores = cross_val_score(self.model, X_train, y_train, cv=20, scoring='neg_mean_squared_error')
        cv_rmse = np.sqrt(-cv_scores.mean())
        logging.info(f"20-Fold Cross-Validation RMSE: {cv_rmse:.4f}")
        ...
        # Sau tuning (nếu dùng Random Forest), dự đoán trên train và test
        y_train_pred = self.model.predict(X_train)
        y_test_pred = self.model.predict(X_test)
