...
model = RandomForestRegressor(
    n_estimators=1000,  # Số lượng cây trong rừng
    max_depth=12,       # Độ sâu tối đa của mỗi cây
    random_state=42,
    n_jobs=-1           # Sử dụng tất cả các lõi CPU để huấn luyện
)
print("Training Random Forest model...")

# Huấn luyện mô hình và log sau mỗi 100 bước
for i in range(1, model.n_estimators + 1):
    model.set_params(n_estimators=i)
    model.fit(X_train, y_train)

    if i % 100 == 0:
        print(f"\nEvaluation at {i} estimators:")
